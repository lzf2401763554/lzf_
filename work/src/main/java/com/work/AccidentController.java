...
// 判断返回的按钮类型是确定还是取消，再据此分别进一步处理
     ...
     * @return 返回读入数据
     */
    public  List<Accidents> readListFromFile1(File file) {
        ...
        List<Accidents> list1 = new ArrayList<>();
        for (String s:list){
     ...
     * @param file 目标文件
     */
    public void writeListToFile2(List<Accidents> list,File file) {
     ...
     * @return 返回读入数据
     */
    public List<Accidents> readListFromFile2(File file) {
        List<Accidents> list = null;
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
            list = (List<Accidents>) ois.readObject();
     ...
     * @param file 目标文件
     */
    public  void writeListToExcel(List<Accidents> dataList,File file) {
     ...
     * @return 返回读入数据
     */
    public  List<Accidents> readListFromExcel(File file) {
    ...
    public  class ExcelDataListener extends AnalysisEventListener<Accidents> {
        private List<Accidents> dataList;

        public ExcelDataListener(List<Accidents> dataList) {
            this.dataList = dataList;
        }


        @Override
        public void invoke(Accidents data, AnalysisContext context) {
