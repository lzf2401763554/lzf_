 ...
 * 维修管理系统控制类
 */
public class MaintenanceController <T>{
...
// 判断返回的按钮类型是确定还是取消，再据此分别进一步处理
    ...
    private void import_() throws SQLException {
        ...
        if (selectedFile.getName().contains(".txt"))list=(List<Maintenance>) readListFromFile1(selectedFile);
        if (selectedFile.getName().contains(".data"))list=(List<Maintenance>) readListFromFile2(selectedFile);
        if (selectedFile.getName().contains(".xlsx"))list=(List<Maintenance>) readListFromExcel(selectedFile);
        tableView.getItems().clear();
        pageTable.setVisible(false);
        String num1 = "";
        String num2 = "";
            ...
            if (result.get() == ButtonType.OK) {
                ...
                for (String s:num3){
                    if (!roadsJDBC.getByCondition(" where road_id="+map.get(Integer.parseInt(s)).getRoad_id()).isEmpty()){
                        maintenanceJDBC.updateByKey(Integer.parseInt(s), map.get(Integer.parseInt(s)));
                    }
                    else num4+=s+",";
                }
                if (!num4.isEmpty()) {
                    Alert alert4 = new Alert(Alert.AlertType.WARNING);
                    alert4.setHeaderText("维修ID:" + num4 + "辖区权限不足！");
                    alert4.showAndWait();
                }
            }
        }
